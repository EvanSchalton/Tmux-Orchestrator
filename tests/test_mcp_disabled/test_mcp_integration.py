"""Integration tests for FastMCP server and tool execution.

This test suite validates:
1. FastMCP server initialization and tool registration
2. Phase 1 tool execution and validation
3. MCP protocol compliance and performance
4. Real client communication scenarios
...
class TestFastMCPServerIntegration:
    ...
    async def test_server_initialization(self):
        ...
        assert mcp.name == "tmux-orchestrator"
    ...
    @pytest.mark.asyncio
    async def test_phase1_tools_registration(self):
        """Test that all Phase 1 tools are properly registered."""
        tools = await mcp.list_tools()
        tool_names = [tool.name for tool in tools]

        expected_tools = ["spawn_agent", "send_message", "get_agent_status", "kill_agent"]

        for expected_tool in expected_tools:
            assert expected_tool in tool_names, f"Tool {expected_tool} not found in {tool_names}"

    @pytest.mark.asyncio
    async def test_tool_signatures_validation(self):
        """Test that all Phase 1 tools have valid signatures."""
        tools = await mcp.list_tools()
        ...
        for tool in tools:
            ...
            assert tool.name is not None and tool.name.strip() != ""

            # Input schema should be valid JSON schema
            schema = tool.inputSchema
            assert isinstance(schema, dict), "inputSchema should be a dictionary"
            assert "type" in schema, "inputSchema should have type field"

    @pytest.mark.asyncio
    async def test_performance_requirements(self):
        """Test FastMCP server meets performance requirements."""
        ...
        # Should complete in under 1 second as per requirements
        assert elapsed < 1.0, f"Server operations took {elapsed:.3f}s, expected <1.0s"
