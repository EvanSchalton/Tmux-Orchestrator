...
class TestMessageDetection(unittest.TestCase):
    ...
    def test_agent_test_message_not_submitted(self):
        ...
        self.assertTrue(has_unsubmitted_message(content), "Failed to detect 'Another test message' as unsubmitted")

        # Should detect state as MESSAGE_QUEUED
        state = detect_agent_state(content)
        self.assertEqual(state, AgentState.MESSAGE_QUEUED, f"Expected MESSAGE_QUEUED but got {state}")
    ...
    def test_all_message_queued_fixtures(self):
        ...
        for fixture_file in message_queued_dir.glob("*.txt"):
            with self.subTest(fixture=fixture_file.name):
                ...
                self.assertTrue(has_msg, f"{fixture_file.name}: Failed to detect unsubmitted message")

                # Should detect state as MESSAGE_QUEUED
                state = detect_agent_state(content)
                self.assertEqual(
                    state, AgentState.MESSAGE_QUEUED, f"{fixture_file.name}: Expected MESSAGE_QUEUED but got {state}"
                )
