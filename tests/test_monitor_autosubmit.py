...
class MonitorAutoSubmitTester:
    ...
    def test_single_agent_stuck(self):
        ...
        auto_submit_found = self.check_log_for_pattern(
            f"Auto-submitting stuck message for {session_name}:0",
            since_time=test_start
        )
        
        idle_detected = self.check_log_for_pattern(
            f"Agent {session_name}:0 is idle with Claude interface",
            since_time=test_start
        )
    ...
    def test_multiple_agents_stuck(self):
        ...
        for session in sessions:
            detected = self.check_log_for_pattern(
                f"Auto-submitting stuck message for {session}:0",
                since_time=test_start
            )
    ...
    def test_repeated_stuck_agent(self):
        ...
        pm_notified = self.check_log_for_pattern(
            f"still stuck after",
            since_time=test_start
        )
    ...
    def test_monitor_stability(self):
        ...
        _, pid_output, _ = self.run_command("cat /workspaces/Tmux-Orchestrator/.tmux_orchestrator/idle-monitor.pid 2>/dev/null")
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w') as f:
            f.write(f"# Monitor Auto-Submit Test Report\n\n")
