...
def test_broadcast_to_team_agent_detection(window_names: list[str], expected_agent_count: int) -> None:
    ...
    mock_windows: list[dict[str, str]] = [{"index": str(i), "name": name} for i, name in enumerate(window_names)]
    mock_tmux.list_windows.return_value = mock_windows
