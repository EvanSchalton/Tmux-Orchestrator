    ...
    def test_message_queued_fixtures_have_unsubmitted_messages(self):
        ...
        for fixture_path in self.fixtures["message_queued"]:
            with self.subTest(fixture=fixture_path.name):
                content = fixture_path.read_text()
                self.assertTrue(
                    has_unsubmitted_message(content), f"File {fixture_path.name} should have unsubmitted message"
                )
    ...
    def test_idle_fixtures_have_no_unsubmitted_messages(self):
        ...
        for fixture_path in self.fixtures["idle"]:
            with self.subTest(fixture=fixture_path.name):
                content = fixture_path.read_text()
                self.assertFalse(
                    has_unsubmitted_message(content), f"File {fixture_path.name} should not have unsubmitted message"
                )
    ...
    def test_crashed_fixtures_have_no_claude_interface(self):
        ...
        for fixture_path in self.fixtures["crashed"]:
            with self.subTest(fixture=fixture_path.name):
                content = fixture_path.read_text()
                self.assertFalse(
                    is_claude_interface_present(content), f"File {fixture_path.name} should not have Claude interface"
                )
        ...
        for state_name, fixture_files in self.fixtures.items():
            for fixture_path in fixture_files:
                with self.subTest(state=state_name, fixture=fixture_path.name):
                    # Basic naming checks
                    self.assertTrue(
                        fixture_path.name.endswith(".txt"), f"Fixture {fixture_path.name} should end with .txt"
                    )
                    # Ensure no temporary or backup files
                    self.assertNotIn(
                        ".bak", fixture_path.name, f"Backup file {fixture_path.name} should not be in fixtures"
                    )
                    self.assertNotIn(
                        ".tmp", fixture_path.name, f"Temporary file {fixture_path.name} should not be in fixtures"
                    )
                    self.assertNotIn(
                        ".corrupted", fixture_path.name, f"Corrupted file {fixture_path.name} should not be in fixtures"
                    )
