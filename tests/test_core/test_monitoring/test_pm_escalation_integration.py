...
"""
        ...
        # Mock _discover_agents to return our test agents
        with patch.object(
            monitor,
            "_discover_agents",
            return_value=["test-session:0", "test-session:1", "test-session:2", "test-session:3"],
        ):
            # Mock logger
            with patch("tmux_orchestrator.core.monitor.logging.getLogger") as mock_get_logger:
                ...
                with patch.object(monitor, "_collect_notification", side_effect=capture_notification):
                    # Pre-populate idle agents to simulate them being detected as idle
                    monitor._idle_agents = {
                        "test-session:0": base_time,
                        "test-session:1": base_time,
                        "test-session:2": base_time,
                        "test-session:3": base_time,
                    }
    ...
    def test_notification_collection_and_delivery(self, monitor, mock_tmux):
        ...
        with patch("tmux_orchestrator.core.monitor.logging.getLogger") as mock_get_logger:
            ...
            # Mock finding PM agents
            def mock_find_pm_in_session(tmux, session):
                return f"{session}:0"  # PM is always in window 0

            with patch.object(monitor, "_find_pm_in_session", side_effect=mock_find_pm_in_session):
                pm_notifications = {}
                monitor._check_team_idleness(mock_tmux, all_agents, mock_logger, pm_notifications)
    ...
    def test_escalation_message_content(self, monitor, mock_tmux, threshold, expected_message_fragment):
        ...
        # Pre-populate escalation history for previous thresholds
        if threshold > 3:
            monitor._pm_escalation_history["test-session:0"] = {3: datetime.now() - timedelta(minutes=threshold - 3)}
