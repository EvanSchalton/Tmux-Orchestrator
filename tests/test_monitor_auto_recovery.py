...
class TestAutoRecoveryMechanisms:
    ...
    def test_crashed_agent_auto_restart(self):
        ...
        self.mock_tmux.capture_pane.side_effect = [crashed_content] * 10  # Enough for all snapshot calls
    ...
    def test_restart_failure_notifies_pm(self):
        ...
        self.mock_tmux.capture_pane.side_effect = [crashed_content] * 10
        ...
        with patch.object(self.monitor, "_attempt_agent_restart", return_value=False):
            with patch.object(self.monitor, "_find_pm_target", return_value=pm_target):
                with patch.object(self.monitor, "_notify_crash") as mock_notify:
                    with patch("tmux_orchestrator.core.monitor.is_claude_interface_present", return_value=False):
                        with patch("time.sleep"):
                            self.monitor._check_agent_status(self.mock_tmux, target, self.logger)

                    # Verify _notify_crash was called
                    mock_notify.assert_called_once_with(self.mock_tmux, target, self.logger)
    ...
    def test_restart_command_sequence(self):
        ...
        self.mock_tmux.press_ctrl_c.assert_called_once_with(target)

        # Then claude command
        self.mock_tmux.send_text.assert_called_once_with(target, "claude --dangerously-skip-permissions")

        # Verify Enter was pressed after the command
        press_enter_calls = self.mock_tmux.press_enter.call_args_list
    ...
    def test_restart_cooldown_mechanism(self):
        ...
        assert not self.mock_tmux.send_text.called
...
Loading configuration..."""
    ...
    def test_recovery_exception_handling(self):
        ...
        self.mock_tmux.capture_pane.side_effect = ["bash $ "] * 10
    ...
    def test_concurrent_recovery_protection(self):
        ...
        for _ in range(5):
            ...
            with patch("tmux_orchestrator.core.monitor.is_claude_interface_present", return_value=False):
                with patch.object(self.monitor, "_attempt_agent_restart", return_value=False):
                    with patch.object(self.monitor, "_notify_crash"):
                        with patch("time.sleep"):
                            self.monitor._check_agent_status(self.mock_tmux, target, self.logger)
