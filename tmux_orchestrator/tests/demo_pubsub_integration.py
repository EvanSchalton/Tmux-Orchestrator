...
async def demo_pubsub_integration():
    ...
    success = pubsub.publish_recovery_needed(
        agent="frontend-dev:3",
        issue="Unresponsive for 15 minutes",
        session_name=session_name,
    ...
    success = pubsub.publish_fresh_agent(
        agent="qa-engineer:4",
        session_name=session_name,
    ...
    success = pubsub.publish_team_idle(
        session_name=session_name,
        idle_agents=["backend-dev:2", "frontend-dev:3", "docs-writer:5"],
        total_agents=5
    )
    print(f"   Result: {'✅ Sent' if success else '❌ Failed'}")
    await asyncio.sleep(0.1)
    ...
    for i in range(12):
        pubsub.publish_monitoring_report(
            ...
            summary={"active": 3, "idle": 1, "crashed": 0},
            issues=[]
        )
    ...
    print(f"  • Priority-based message routing")
    print(f"  • Structured message format")
    print(f"  • Automatic batching for low priority")
    print(f"  • Sub-100ms delivery targets")
