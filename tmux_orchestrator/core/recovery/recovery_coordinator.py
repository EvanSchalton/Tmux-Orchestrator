...
def coordinate_agent_recovery(
    ...
    try:
        ...
        if elapsed_time >= recovery_timeout:
            timeout_message: str = (
                f"Recovery timeout reached for {target} "
                f"({elapsed_time:.1f}s >= {recovery_timeout}s)"
            )
        ...
        try:
            from tmux_orchestrator.core.recovery.notification_manager import send_recovery_notification
            
            notification_sent, notification_msg, notification_data = send_recovery_notification(
                tmux=tmux,
                target=target,
        ...
        if restart_success and recovery_data.get('recovery_verified', True):
            final_message: str = (
                f"Recovery successful for {target} "
                f"in {total_duration:.1f}s: {restart_message}"
            )
            ...
            final_error: str = (
                f"Recovery failed for {target} "
                f"after {total_duration:.1f}s: {restart_message}"
            )
        ...
        exception_message: str = (
            f"Recovery coordination failed for {target} "
            f"after {error_duration:.1f}s: {str(e)}"
        )
