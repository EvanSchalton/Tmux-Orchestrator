...
class RecoveryDaemon:
    ...
    async def _discover_agents(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if self.tmux.has_session("tmux-orc-dev"):
                dev_agents = [
                    {"target": "tmux-orc-dev:1", "role": "orchestrator", "session": "tmux-orc-dev", "window": "1"},
                    {"target": "tmux-orc-dev:2", "role": "developer", "session": "tmux-orc-dev", "window": "2"},
                    {"target": "tmux-orc-dev:3", "role": "developer", "session": "tmux-orc-dev", "window": "3"},
                    {"target": "tmux-orc-dev:4", "role": "developer", "session": "tmux-orc-dev", "window": "4"},
                    {"target": "tmux-orc-dev:5", "role": "pm", "session": "tmux-orc-dev", "window": "5"},
                ]
    ...
    def _identify_recovery_candidates(self, health_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for result in health_results:
            ...
            if result['consecutive_failures'] < self.failure_threshold:
                self.logger.debug(f"Skipping {target} - insufficient failures ({result['consecutive_failures']}/{self.failure_threshold})")
                continue
    ...
    def _update_agent_states(self, health_results: List[Dict[str, Any]]) -> None:
        ...
        for result in health_results:
            ...
            self.agent_states[target] = {
                ...
                'last_response': result['check_time'] if not result['is_failed'] else 
                               self.agent_states.get(target, {}).get('last_response', result['check_time'])
            }
    ...
    def _log_daemon_stats(self, last_cycle_duration: float) -> None:
        ...
        stats = {
            ...
            'success_rate': (
                (self.total_recoveries_successful / max(self.total_recoveries_attempted, 1)) * 100
            ),
