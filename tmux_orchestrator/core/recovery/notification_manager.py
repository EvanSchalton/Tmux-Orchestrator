...
def should_send_recovery_notification(
    ...
    valid_types: List[str] = ['recovery_started', 'recovery_failed', 'recovery_success', 'agent_healthy']
    ...
    try:
        ...
        return True, f"State error, allowing notification: {error_msg}", notification_data
...
def send_recovery_notification(
    ...
    try:
        ...
        if not force_send:
            ...
            should_send, cooldown_reason, cooldown_data = should_send_recovery_notification(
                target=target,
                notification_type=notification_type,
                cooldown_minutes=cooldown_minutes,
                logger=logger
        ...
        script_path: str = "/workspaces/Tmux-Orchestrator/send-claude-message.sh"
        
        import subprocess
        result = subprocess.run([
            script_path, pm_target, formatted_message
...
def _format_recovery_message(
    ...
    formatted_parts = [
        ...
        message,
        f"",
        f"This is an automated recovery system notification."
    ]
