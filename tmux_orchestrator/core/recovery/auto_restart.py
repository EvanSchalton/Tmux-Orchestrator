...
def auto_restart_agent(
    ...
    if preserve_context:
        try:
            ...
            if conversation_content:
                ...
                context_file: Path = _save_context_to_file(
                    target=target,
    ...
    if not restart_success:
        final_message: str = (
            f"Auto-restart failed for {target} after {max_retries} attempts: {restart_message}"
        )
    ...
    if restart_success:
        try:
            from tmux_orchestrator.core.recovery.briefing_manager import restore_agent_briefing
    ...
    success_message: str = (
        f"Agent {target} auto-restarted successfully "
        f"(attempt {context_data['retry_attempts']}/{max_retries})"
    )
...
def _save_context_to_file(
    ...
    try:
        ...
        with open(context_file, 'w', encoding='utf-8') as f:
            f.write(f"=== Agent Context Recovery Log ===\n")
