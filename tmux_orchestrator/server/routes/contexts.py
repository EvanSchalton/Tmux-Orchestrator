...
# Contexts directory
CONTEXTS_DIR = Path("/workspaces/Tmux-Orchestrator/.tmux_orchestrator/contexts")
...
class ContextListResponse(BaseModel):
    ...
    note: str = "Only system roles (orchestrator, pm) have standard contexts. Other agents should have custom briefings."
...
async def get_context(role: str):
    ...
    if not context_file.exists():
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Context '{role}' not found. Available: {', '.join(available)}"
        )
...
async def spawn_with_context(role: str, session: str, extend: str = None):
    ...
    if not context_file.exists():
        raise HTTPException(
            status_code=404,
            detail=f"Context '{role}' not found. Only system roles have standard contexts."
        )
    ...
    if not success:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to spawn {role} agent at {session}"
        )
    
    return {
        "success": True,
        "message": f"Spawned {role} agent at {session}",
        "session": session,
        "role": role
    }