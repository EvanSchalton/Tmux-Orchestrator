...
class Task(BaseModel):
    ...
    assigned_to: Optional[str] = Field(
        None, 
        description="Agent target assigned to this task",
        pattern=r"^[^:]+:[^:]+$"
    )
    status: str = Field(
        "pending", 
...
class TaskQueue(BaseModel):
    ...
    queue_metrics: Dict[str, int] = Field(
        ..., 
        description="Queue statistics and metrics"
    )
...
class TaskCreateResponse(BaseModel):
    ...
    queue_position: Optional[int] = Field(
        None, 
        description="Position in agent's task queue"
    )
...
class TaskStatusRequest(BaseModel):
    ...
    include_history: bool = Field(
        False, 
        description="Include task history/changelog"
    )
...
class TaskStatusResponse(BaseModel):
    ...
    progress_details: Dict[str, Any] = Field(
        ..., 
    ...
    history: List[Dict[str, Any]] = Field(
        default_factory=list, 
        description="Task history/changelog"
    )
