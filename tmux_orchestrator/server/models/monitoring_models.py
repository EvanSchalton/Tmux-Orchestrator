...
class SystemStatusResponse(BaseModel):
    ...
    resource_usage: Dict[str, Union[float, int]] = Field(
        ..., 
...
class SessionDetailRequest(BaseModel):
    ...
    session_name: str = Field(
        ..., 
        description="Session name to get details for",
        min_length=1
    )
    include_agent_activity: bool = Field(
        True, 
        description="Include recent agent activity"
    )
    ...
    activity_lines: int = Field(
        50, 
        description="Number of activity lines per agent",
        ge=1,
        le=200
    )
...
class SessionDetailResponse(BaseModel):
            ...
            "agents": [
                {
                    ...
                    "status": "active",
                    "uptime_minutes": 90,
                    "recent_activity": ["Working on login component", "Running tests"],
                    "performance": {
    ...
    session_metadata: Dict[str, Any] = Field(
        ..., 
        description="Session metadata and basic information"
    )
    agents: List[Dict[str, Any]] = Field(
        ..., 
        description="Detailed agent information"
    )
    session_performance: Dict[str, Union[float, int]] = Field(
        ..., 
        description="Session-level performance metrics"
    )
...
class IdleAgentsRequest(BaseModel):
    ...
    min_idle_minutes: int = Field(
        5, 
        description="Minimum minutes an agent must be idle",
        ge=0,
        le=1440
    )
    include_activity_context: bool = Field(
        True, 
        description="Include last activity information"
    )
    group_by_session: bool = Field(
        False, 
        description="Group results by session"
    )
...
class IdleAgentsResponse(BaseModel):
    ...
    idle_agents: List[Dict[str, Any]] = Field(
        ..., 
        description="List of idle agents with details"
    )
    total_idle: int = Field(..., description="Total number of idle agents")
    by_session: Dict[str, int] = Field(
        ..., 
        description="Idle agents grouped by session"
    )
...
class ActiveAgentsRequest(BaseModel):
    ...
    include_current_task: bool = Field(
        True, 
    ...
    activity_threshold_minutes: int = Field(
        2, 
        description="Minutes of activity to consider 'active'",
        ge=0,
        le=60
    )
...
class ActiveAgentsResponse(BaseModel):
    ...
    active_agents: List[Dict[str, Any]] = Field(
        ..., 
        description="List of active agents with details"
    )
    total_active: int = Field(..., description="Total number of active agents")
    by_activity_level: Dict[str, int] = Field(
        ..., 
        description="Active agents grouped by activity level"
    )
