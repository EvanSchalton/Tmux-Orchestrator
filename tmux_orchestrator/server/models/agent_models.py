...
class AgentStatusRequest(BaseModel):
    ...
    include_activity: bool = Field(True, description="Include recent activity in response")
    activity_lines: int = Field(50, description="Number of activity lines to capture", ge=1, le=200)
...
class AgentKillRequest(BaseModel):
    ...
    target: str = Field(..., description="Agent target to terminate", pattern=r"^[^:]+:[^:]+$")
    force: bool = Field(False, description="Force termination without graceful shutdown")
    save_state: bool = Field(True, description="Save agent state before termination")
