...
class TeamMember(BaseModel):
    ...
    briefing: Optional[str] = Field(
        None, 
        description="Custom briefing for this team member",
        max_length=1000
    )
...
class TeamDeploymentRequest(BaseModel):
    ...
    team_members: List[TeamMember] = Field(
        ..., 
        description="List of team members to deploy",
        min_length=1,
        max_length=20
    )
...
class TeamDeploymentResponse(BaseModel):
    ...
    deployed_agents: List[Dict[str, str]] = Field(
        ..., 
        description="List of successfully deployed agents"
    )
    coordination_setup: Dict[str, Any] = Field(
        ..., 
        description="Coordination pattern configuration"
    )
...
class StandupRequest(BaseModel):
    ...
    session_names: List[str] = Field(
        ..., 
        description="List of team sessions to include in standup",
        min_length=1
    )
    include_idle_agents: bool = Field(
        True, 
...
class StandupResponse(BaseModel):
    ...
    results: List[Dict[str, Any]] = Field(
        ..., 
        description="Results per session"
    )
    total_agents_contacted: int = Field(
        ..., 
        description="Total number of agents contacted"
    )
...
class TeamRecoveryRequest(BaseModel):
    ...
    session_name: str = Field(
        ..., 
        description="Team session to recover",
        min_length=1
    )
    ...
    preserve_context: bool = Field(
        True, 
        description="Attempt to preserve agent work context"
    )
    max_parallel_recoveries: int = Field(
        3, 
        description="Maximum agents to recover simultaneously",
        ge=1,
        le=10
    )
...
class TeamRecoveryResponse(BaseModel):
    ...
    recovery_summary: Dict[str, int] = Field(
        ..., 
        description="Summary statistics of recovery operation"
    )
    recovered_agents: List[str] = Field(
        ..., 
        description="List of successfully recovered agents"
    )
    recovery_failures: List[str] = Field(
        ..., 
        description="List of agents that failed to recover"
    )
...
class TeamStatusRequest(BaseModel):
    ...
    session_name: str = Field(
        ..., 
        description="Team session to check status for",
        min_length=1
    )
    ...
    activity_window_minutes: int = Field(
        60, 
        description="Minutes of activity history to include",
        ge=5,
        le=1440
    )
...
class TeamStatusResponse(BaseModel):
    ...
    coordination_status: Dict[str, Any] = Field(
        ..., 
        description="Team coordination health information"
    )
