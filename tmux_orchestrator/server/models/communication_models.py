...
class BroadcastResponse(BaseModel):
    ...
    results: List[Dict[str, Union[str, bool]]] = Field(
        ..., 
        description="Detailed delivery results per target"
    )
...
class InterruptRequest(BaseModel):
    ...
    target: str = Field(
        ..., 
        description="Target agent to interrupt",
        pattern=r"^[^:]+:[^:]+(\.\d+)?$"
    )
    interrupt_type: str = Field(
        "soft", 
        description="Type of interrupt",
        pattern="^(soft|hard|emergency)$"
    )
    ...
    new_instruction: Optional[str] = Field(
        None, 
        description="New instruction after interrupt",
        max_length=1000
    )
...
class ConversationHistoryRequest(BaseModel):
    ...
    lines: int = Field(
        50, 
        description="Number of conversation lines to retrieve",
        ge=1,
        le=1000
    )
    include_timestamps: bool = Field(
        True, 
        description="Include timestamps in history"
    )
...
class ConversationHistoryResponse(BaseModel):
    ...
    history_range: Dict[str, str] = Field(
        ..., 
        description="Timestamp range of conversation history"
    )
