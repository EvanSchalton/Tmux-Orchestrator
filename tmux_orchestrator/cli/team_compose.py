...
TEAM_TEMPLATE = Path.home() / "workspaces" / "Tmux-Orchestrator" / ".tmux_orchestrator" / "templates" / "team-composition-template.md"
...
def compose(project_name: str, prd: Optional[str], interactive: bool, template: Optional[str]) -> None:
    ...
    if team_comp_path.exists():
        if not Confirm.ask(f"Team composition already exists. Overwrite?"):
            return
...
def _interactive_team_composition(templates: Dict, project_name: str, prd: Optional[str]) -> List[Dict]:
    ...
    for i in range(team_size):
        ...
        # Get customization
        focus = Prompt.ask(f"Specific focus for this {templates[template_name]['role']}", 
...
def _suggest_team_composition(project_name: str, prd: Optional[str], templates: Dict) -> List[Dict]:
    ...
    is_fullstack = 'fullstack' in project_name.lower() or 'full-stack' in project_name.lower()
...
def _generate_team_composition(project_name: str, agents: List[Dict], 
                              output_path: Path, prd: Optional[str]) -> None:
    """Generate team composition document."""
    ...
    team_members = []
    for agent in agents:
        member = f"""### {agents.index(agent) + 1}. {agent['role']}
...
- **Key Responsibilities**:
  ...
  - Project-specific focus on {agent['focus']}"""
    ...
    content = content.replace("{Explain why this specific team composition was chosen, what project aspects drove the decisions}", rationale)
...
def _load_team_template(template_name: str) -> List[Dict]:
    ...
    templates = {
        'api-heavy': [
            {'role': 'Project Manager', 'template': 'project-manager', 'window': 0, 
             'focus': 'API project coordination'},
            {'role': 'API Designer', 'template': 'api-designer', 'window': 1,
             'focus': 'RESTful API architecture'},
            {'role': 'Backend Developer', 'template': 'backend-developer', 'window': 2,
             'focus': 'API implementation'},
            {'role': 'Backend Developer', 'template': 'backend-developer', 'window': 3,
             'focus': 'Database and business logic'},
            {'role': 'Test Automation', 'template': 'test-automation', 'window': 4,
             'focus': 'API testing'},
        ],
        'cli-tool': [
            {'role': 'Project Manager', 'template': 'project-manager', 'window': 0,
             'focus': 'CLI project coordination'},
            {'role': 'CLI Developer', 'template': 'cli-developer', 'window': 1,
             'focus': 'Command structure and parsing'},
            {'role': 'CLI Developer', 'template': 'cli-developer', 'window': 2,
             'focus': 'Terminal UI and output'},
            {'role': 'Technical Writer', 'template': 'technical-writer', 'window': 3,
             'focus': 'CLI documentation'},
            {'role': 'QA Engineer', 'template': 'qa-engineer', 'window': 4,
             'focus': 'CLI testing'},
        ]
    }
...
def deploy(project_name: str, custom: bool) -> None:
    ...
    if custom or team_comp.exists():
        ...
        ctx.invoke(team_cli.commands['deploy-standard'], 
                  team_type='fullstack', 
                  size=5, 
