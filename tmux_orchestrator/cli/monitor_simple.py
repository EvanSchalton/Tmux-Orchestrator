...
def start(interval: int):
    ...
    pid_file = Path("/tmp/tmux-orchestrator-simple-monitor.pid")
    ...
    if pid == 0:
        ...
        console.print("  Log file: /tmp/tmux-orchestrator-simple-monitor.log")
...
def status():
    ...
    try:
        ...
        log_file = Path("/tmp/tmux-orchestrator-simple-monitor.log")
        if log_file.exists():
